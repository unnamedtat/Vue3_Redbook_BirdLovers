...
from Server01.util.auxiliaryFuction import check_email, combine_index_post, check_and_delete, filter_querySet, \
    get_user_post_info, get_user_info, post_to_picgo
...
def update_avatar(request, payload):
    ...
    id = payload['user_id']
    img_uuid_seed = f'{id}-{file.name}'
    upload_result = post_to_picgo(file, img_uuid_seed, post_type='avatar')
    if upload_result['status']== 'success':
        user = models.User.objects.filter(id=id).first()
        user.avatar = upload_result['url']
        user.save()
        return JsonResponse({'info': upload_result['url']}, status=200)
    return JsonResponse({'error': upload_result['message']}, status=500)
