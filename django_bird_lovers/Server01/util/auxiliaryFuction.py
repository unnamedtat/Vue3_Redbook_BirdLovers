...
# 上传到图床
def post_to_picgo(file, img_uuid_seed, post_type='post'):
    namespace = uuid.NAMESPACE_OID
    file_extension = os.path.splitext(file.name)[1]
    file_name = str(uuid.uuid5(namespace, img_uuid_seed + str(datetime.now().timestamp()))) + file_extension
    upload_result = ImageUploadUtil.upload_image(file, file_name, post_type)
